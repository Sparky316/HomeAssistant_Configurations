...
_LOG = logging.getLogger(__name__ + "_qse")
...
_PROMPT_ENABLE = b'#MONITORING,12,1' + _EOL
_PROMPT_ENABLE_SUCCESS = b'~MONITORING,12,1' + _EOL
...
class QSE(object):
    ...
    def __init__(self, hostname=None, username=_DEFAULT_USERNAME):
        ...
        # The following line makes it wait forever:
    ...
    def _login(self):
        ...
        try:
            ...
            _LOG.debug(" Got telnet response: [%s]", str(response))
            ...
            progressNum = 6
            # It was found that sometimes the prompt can be disabled, and then
            # Lutron will not give the 'QSE>' prompt.
            # To make sure it's on, Here we tell it to turn on the prompt
            #telnet.write(bytes(_PROMPT_ENABLE, encoding='ascii') + _EOL)
            telnet.write(_PROMPT_ENABLE)
            progressNum = 7
            response = telnet.read_until(_PROMPT_ENABLE_SUCCESS, timeout=_TIMEOUT)
            progressNum = 8
            _LOG.debug(" Got Prompt-enable response: [%s]", str(response))
            assert response == _PROMPT_ENABLE_SUCCESS, response
            
            
        except (socket.error, socket.gaierror, socket.timeout, EOFError):
            #_LOG.error('Failed to connect to Lutron QSE.'
            #           ' Retry in %s seconds.', str(_LOGIN_ATTEMPT_INTERVAL))
            _LOG.error('Failed to connect to Lutron QSE.')
            if progressNum == 1:
                _LOG.error('   --> Likely bad Host IP address: %s', str(self._hostname) )    
            elif progressNum == 3:
                _LOG.error('   --> Stuck waiting for login prompt: %s', str(_LOGIN_PROMPT) )    
            elif progressNum == 5:
                _LOG.error('   --> Stuck waiting for login response: %s', str(_LOGIN_SUCCESS) )    
            elif progressNum == 7:
                _LOG.error('   --> Stuck waiting for Prompt Enable response: %s', str(_PROMPT_ENABLE_SUCCESS) )    
                
            _LOG.error('   Retry in %s', str(_LOGIN_ATTEMPT_INTERVAL))
    ...
    def _load_devices(self):
        ...
        _LOG.debug("Done with load_devices" )
    ...
    def _init_device(self, response):
        ...
        if product in _DETAILS_PRODUCT_ROLLERS:
            ...
            _LOG.info(" Found Roller: %s (SN: %s)", str(integration_id.decode('utf-8', 'ignore')), str(serial_number.decode('utf-8', 'ignore')) )
        else:
            _LOG.info(" Found Device: [%s] (SN: %s)",  str(product.decode('utf-8', 'ignore')), str(serial_number.decode('utf-8', 'ignore')) )
